...
/**
 * Lazy lazy lazy. We don't unpack the underlying class unless necessary. Things like new methods and annotations
 * accumulate in here until they must be written out, don't add them to the underlying MethodGen! Things are slightly
...
private static final Type[] PARAMSIGNATURE_MAKESJP_METHOD =
        new Type[] {Type.STRING, Type.INT, Type.STRING, Type.CLASS, Type.CLASS_ARRAY, Type.STRING_ARRAY,
                Type.CLASS_ARRAY, Type.CLASS, Type.INT};
private static final Type[] PARAMSIGNATURE_MAKESJP_CONSTRUCTOR =
        new Type[] {Type.STRING, Type.INT, Type.CLASS, Type.CLASS_ARRAY, Type.STRING_ARRAY, Type.CLASS_ARRAY, Type.INT};
private static final Type[] PARAMSIGNATURE_MAKESJP_CATCHCLAUSE =
        new Type[] {Type.STRING, Type.CLASS, Type.CLASS, Type.STRING, Type.INT};
private static final Type[] PARAMSIGNATURE_MAKESJP_FIELD =
        new Type[] {Type.STRING, Type.INT, Type.STRING, Type.CLASS, Type.CLASS, Type.INT};
private static final Type[] PARAMSIGNATURE_MAKESJP_INITIALIZER =
        new Type[] {Type.STRING, Type.INT, Type.CLASS, Type.INT};
private static final Type[] PARAMSIGNATURE_MAKESJP_MONITOR = new Type[] {Type.STRING, Type.CLASS, Type.INT};
private static final Type[] PARAMSIGNATURE_MAKESJP_ADVICE =
        new Type[] {Type.STRING, Type.INT, Type.STRING, Type.CLASS, Type.CLASS_ARRAY, Type.STRING_ARRAY,
                Type.CLASS_ARRAY, Type.CLASS, Type.INT};
...
private final Map<BcelShadow, Field> tjpFields = new HashMap<>();
...
public LazyClassGen(BcelObjectType myType) {
    ...
    if (implementsSerializable(getType())) {
        ...
        if (!getType().isInterface() && !hasSerialVersionUIDField && world.isAddSerialVerUID()) {
            ...
            FieldGen fg =
                    new FieldGen(Constants.ACC_PRIVATE | Constants.ACC_FINAL | Constants.ACC_STATIC, BasicType.LONG,
            ...
            if (world.getLint().calculatingSerialVersionUID.isEnabled()) {
                world.getLint().calculatingSerialVersionUID
                        .signal(new String[] {getClassName(), calculatedSerialVersionUID + "L"}, null,
                                null);
            }
        }
    }
...
public void warnOnModifiedSerialVersionUID(ISourceLocation sourceLocation, String reason) {
    if (isSerializable && !hasSerialVersionUIDField) {
        getWorld().getLint().needsSerialVersionUIDField
                .signal(new String[] {myType.getResolvedTypeX().getName(), reason}, sourceLocation, null);
    }
}
...
private void writeBack(BcelWorld world) {
    ...
    if (world.isOverWeaving()) {
        if (myGen.hasAttribute(WeaverState.AttributeName) && myType != null && myType.getWeaverState() != null) {
            ...
            myGen.addAttribute(
                    Utility.bcelAttribute(new AjAttribute.WeaverState(myType.getWeaverState()), getConstantPool()));
        }
    }
    else {
        if (!myGen.hasAttribute(WeaverState.AttributeName) && myType != null && myType.getWeaverState() != null) {
        ...
        if (inlinedFiles.size() != 0) {
            if (hasSourceDebugExtensionAttribute(myGen)) {
                world.showMessage(IMessage.WARNING,
                                  WeaverMessages.format(WeaverMessages.OVERWRITE_JSR45, getFileName()), null, null);
            }
...
/**
 * When working with Java generics, a signature attribute is attached to the type which indicates how it was declared.
 * This routine ensures the signature attribute for the class we are about to write out is correct. Basically its
 * responsibilities are:
 * <ol>
 * <li>
 * Checking whether the attribute needs changing (ie. did weaving change the type hierarchy) - if it did, remove the
 * old
 * attribute
 * <li>
...
/**
 * Retrieve the ajc$preClinit method - this method captures any initialization AspectJ wants to ensure happens in a
 * class. It is called from the static initializer. Maintaining this separation enables overweaving to ignore join
 * points added due to earlier weaves. If the ajc$preClinit method cannot be found, it is created and a call to it is
 * placed in the real static initializer (the call is placed at the start of the static initializer).
 *
...
public LazyMethodGen getAjcPreClinit() {
    ...
    LazyMethodGen ajcPreClinit =
            new LazyMethodGen(Modifier.PRIVATE | Modifier.STATIC, Type.VOID, NameMangler.AJC_PRE_CLINIT_NAME,
                              Type.NO_ARGS, NO_STRINGS, this);
    ...
    if (serialVersionUIDRequiresInitialization) {
        ...
        il.append(getFactory()
                          .createFieldAccess(getClassName(), "serialVersionUID", BasicType.LONG, Constants.PUTSTATIC));
...
/**
 * factory method for building multiple extended clinit methods. Constructs a new clinit method that invokes the
 * previous one and then returns it. The index is used as a name suffix.
 *
...
public LazyMethodGen createExtendedAjcPreClinit(LazyMethodGen previousPreClinit, int i) {
    LazyMethodGen ajcPreClinit =
            new LazyMethodGen(Modifier.PRIVATE | Modifier.STATIC, Type.VOID, NameMangler.AJC_PRE_CLINIT_NAME + i,
                              Type.NO_ARGS, NO_STRINGS, this);
    ajcPreClinit.getBody().insert(InstructionConstants.RETURN);
    methodGens.add(ajcPreClinit);
...
/**
 * Helper method to create a signature attribute based on a string signature: e.g.
 * "Ljava/lang/Object;LI<Ljava/lang/Double;>;"
 */
...
private void initializeTjp(InstructionFactory fact, InstructionList list, Field field, BcelShadow shadow) {
    ...
    boolean isFastSJPAvailable =
            shadow.getWorld().isTargettingRuntime1_6_10() && !enclosingStaticTjpType.equals(field.getType());
    ...
    if (world.isTargettingAspectJRuntime12()) { // TAG:SUPPORTING12: We didn't have optimized factory methods in 1.2
        ...
        if (isFastSJPAvailable) {
            ...
            list.append(
                    fact.createInvoke(factoryType.getClassName(), signatureMakerName, signatureType, Type.STRINGARRAY7,
                                      Constants.INVOKEVIRTUAL));
        }
        ...
        if (w.isJoinpointArrayConstructionEnabled() && sig.getDeclaringType().isArray()) {
            ...
            list.append(
                    fact.createInvoke(factoryType.getClassName(), signatureMakerName, signatureType, Type.STRINGARRAY5,
                                      Constants.INVOKEVIRTUAL));
        }
        else {
            list.append(InstructionFactory.PUSH(cp, makeString(sig.getModifiers(w))));
        ...
        list.append(InstructionFactory.PUSH(cp, makeString(sig.getExceptions(w))));
        list.append(InstructionFactory.PUSH(cp, makeString((sig.getReturnType()))));
        list.append(fact.createInvoke(factoryType.getClassName(), signatureMakerName, signatureType,
                                      new Type[] {Type.STRING, Type.STRING, Type.STRING, Type.STRING, Type.STRING,
                                              Type.STRING, Type.STRING}, Constants.INVOKEVIRTUAL));
    }
    ...
    if (world.isTargettingAspectJRuntime12()) {
        list.append(fact.createInvoke(factoryType.getClassName(), "makeSJP", staticTjpType,
                                      new Type[] {Type.STRING, sigType, Type.INT}, Constants.INVOKEVIRTUAL));
        ...
        if (fastSJP) {
            if (exceptionTypes != null && exceptionTypes.length != 0) {
                list.append(
                        fact.createInvoke(factoryType.getClassName(), factoryMethod, field.getType(), ARRAY_8STRING_INT,
                                          Constants.INVOKEVIRTUAL));
            }
            else {
                list.append(
                        fact.createInvoke(factoryType.getClassName(), factoryMethod, field.getType(), ARRAY_7STRING_INT,
                                          Constants.INVOKEVIRTUAL));
            }
        }
        else {
 ...
 * As of version 1.9.1 the runtime includes new factory methods for joinpoints that take classes, not strings and using
 * them requires different code generation. Using these instead of the old ones means we can avoid deferred
 * classloading
 * for these types. By using the LDC instruction that loads classes, it also means anything modifying woven code and
 * changing type names will also pick up on these references.
 */
...
/**
 * Create a field in the type containing the shadow where the annotation retrieved during binding can be stored - for
 * later fast access.
 *
...
public Field getAnnotationCachingField(BcelShadow shadow, ResolvedType toType, boolean isWithin) {
    ...
    if (field == null) {
        ...
        FieldGen annotationCacheField =
                new FieldGen(Modifier.PRIVATE | Modifier.STATIC, typeForAnnotation, sb.toString(), cp);
...
public void errorOnAddedField(FieldGen field, ISourceLocation sourceLocation) {
    if (isSerializable && !hasSerialVersionUIDField) {
        getWorld().getLint().serialVersionUIDBroken
                .signal(new String[] {myType.getResolvedTypeX().getName(), field.getName()}, sourceLocation, null);
    }
}
...
public LazyMethodGen getLazyMethodGen(String name, String signature, boolean allowMissing) {
    ...
    if (!allowMissing) {
        throw new BCException(
                "Class " + this.getName() + " does not have a method " + name + " with signature " + signature);
    }
...
/**
 * Return the next available field name with the specified 'prefix', e.g. for prefix 'class$' where class$0, class$1
 * exist then return class$2
 */
public String allocateField(String prefix) {
    ...
    return prefix + (highestAllocated + 1);
}
...
static class CacheKey {
    ...
    private final ResolvedType annotationType;

    // If the annotation is being accessed via @annotation on a shadow then we can use the shadows toString() (so two
    // shadows
    // the same share a variable), but if it is @withincode() or @within() we can't share them (as the shadows may
    // look the same
